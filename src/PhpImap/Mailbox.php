...
class Mailbox
{
    ...
    public function __construct($imapPath, $login, $password, $attachmentsDir = null, $serverEncoding = 'UTF-8')
    {
        ...
        if ($attachmentsDir) {
            if (!is_dir($attachmentsDir)) {
                throw new \RuntimeException(sprintf('Directory %s not found', $attachmentsDir ));
            }
    ...
    public function switchMailbox($imapPath = '')
    {
        ...
        if (!$imapStream) {
            throw new \RuntimeException(sprintf('Couldn\'t switch mailbox: %s', imap_last_error()));
        }
    }
    ...
    protected function initImapStream()
    {
        ...
        if (!$imapStream) {
            ...
            imap_errors();
            throw new \RuntimeException(sprintf('Connection error: %s', $lastError));
        }
        return $imapStream;
    }
